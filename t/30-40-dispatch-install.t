...
my $testers = [qw(127.0.0.1 54.200.68.202)];
...
is ($ret[3], '\d\.\d+\.\d+ :: PASS', "pass 1 ok");
is ($ret[4], '\d\.\d+\.\d+ :: PASS', "pass 2 ok");

like ($ret[5], qr/^\d+\.\d+/, "host ok");
like ($ret[6], qr/removing/, "remove ok");
like ($ret[7], qr/installing/, "installing ok");
is ($ret[8], '\d\.\d+\.\d+ :: PASS', "pass 1 ok");
is ($ret[9], '\d\.\d+\.\d+ :: PASS', "pass 2 ok");
