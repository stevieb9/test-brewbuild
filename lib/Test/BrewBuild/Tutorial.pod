...
Auto run a specific number of dispatch runs to the testers. You *must* be
located in a repository directory when dispatching:

    bbdispatch -t 10.1.1.1 -a 10 # run 10 cycles

    bbdispatch -t 10.1.1.1 -a 0  # run forever
