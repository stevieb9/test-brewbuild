...
To mitigate, you can use IP filters, SSH tunnels etc. Although there is
primitive command checking, it's just that; primitive. Be careful.
...
You'll need Git on both the dispatcher and all testers, to be using an
Administrative user (Windows only), your module accessible via a Git repository,
and the dispatch and all tester servers configured, see L<SERVER CONFIGURATION>.
